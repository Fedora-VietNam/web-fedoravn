...
import { TooltipNameType } from "./chart-constants"
import { getPayloadConfigFromPayload } from "./chart-utils"

export const ChartTooltip = RechartsPrimitive.Tooltip

/**
 * @brief Custom content component for chart tooltips.
 */
    ...
    <div className={cn("grid min-w-32 items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
      ...
      <div className="grid gap-1.5">
        {payload.filter((item) => item.type !== "none").map((item, index) => (
          <ChartTooltipItem key={index} item={item} index={index} config={config} indicator={indicator} hideIndicator={hideIndicator} nestLabel={nestLabel} tooltipLabel={tooltipLabel} formatter={formatter} color={color} nameKey={nameKey} />
        ))}
      </div>
    </div>
  )
...
function ChartTooltipItem({ item, index, config, indicator, hideIndicator, nestLabel, tooltipLabel, formatter, color, nameKey }: {
